    ...
    public void runTest(ClientGameTestContext ctx) {
        ...
        try (var server = ctx.worldBuilder().createServer();
            ...
            server.runOnServer(instance -> {
                var level = instance.getOverworld();
                level.setBlockState(CHEST_POS, Blocks.CHEST.getDefaultState(), 3);
                if (level.getBlockEntity(CHEST_POS)
                        instanceof net.minecraft.block.entity.ChestBlockEntity chest) {
                    chest.setStack(0, new net.minecraft.item.ItemStack(
                        net.minecraft.item.Items.STONE, 32));
                    chest.markDirty();
                }
            });
