...
public class WarehouseCrudClientGameTest implements FabricClientGameTest {
    ...
    public void runTest(ClientGameTestContext ctx) {
        ...
        try (var server = ctx.worldBuilder().createServer();
            ...
            ctx.runOnClient(client -> client.setScreen(
            ...
            ctx.runOnClient(client -> {
                ...
                client.setScreen(new GuiWarehouseEditor(created, manager));
            });
            ctx.waitTicks(5);

            ctx.runOnClient(client -> {
                if (client.currentScreen instanceof GuiWarehouseEditor editor) {
    ...
    private WarehouseEntry createWarehouseViaSelectionCallback(ClientGameTestContext ctx) {
        ...
        ctx.runOnClient(client -> {
            if (!(client.currentScreen instanceof GuiWarehouseManager manager)) {
